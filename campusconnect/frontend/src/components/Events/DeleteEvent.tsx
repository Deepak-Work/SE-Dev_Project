...
interface DeleteEventProps {
  ...
  handleDeleteEventClose: (event?: object, reason?: string) => void;
  handleEventClose: (reason: string) => void;
}

const DeleteEvent = (props: DeleteEventProps) => {
  const { deleteEventOpen, handleDeleteEventClose, handleEventClose } = props;

  return (
    <Dialog
      open={deleteEventOpen}
      onClose={handleDeleteEventClose}
      fullWidth
      maxWidth={"sm"}
      PaperProps={{
        sx: {
      ...
      >
        ...
        >
          ...
          <Box sx={{ display: "flex", flexFlow: "column nowrap" }}>
            <Button
              onClick={() => handleEventClose("delete")}
              sx={{
                display: "flex",
                margin: 2,
                border: "2px solid",
            ...
            <Button
              onClick={() => handleDeleteEventClose()}
              sx={{
                display: "flex",
                margin: 2,
                border: "2px solid",
